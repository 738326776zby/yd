...
const NewEvaluationPrincipleModal = ({ open, setOpen,target }: NewEvaluationPrincipleModalProps) => {
  ...
  const formChange=(changedValues:Record<string,string>,allValues:any)=>{
  ...
  return (
    ...
    >
      ...
      >
        <Form.Item label="文件上传" valuePropName="fileList" getValueFromEvent={normFile}>
          <Upload action="/upload.do" >

            <Button icon={<UploadOutlined />}>文件上传</Button>
            <>
              <InfoCircleFilled className='text-sm text-[#FF9F69] ml-4 m-1'/>
              <span className='text-sm text-[#495464]'>支持上传excel或者csv格式文件，5M以内</span>
            </>
          </Upload>
        </Form.Item>
